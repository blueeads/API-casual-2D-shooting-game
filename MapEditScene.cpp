...
bool CMapEditScene::Init()
{
	...
	CTexture* pNoneTex = GET_SINGLE(CResourcesManager)->LoadTexture(
		"TileNone", L"OptionNone.bmp");
	pNoneTex->SetColorKey(255, 0, 255);
	SAFE_RELEASE(pNoneTex);

	CTexture* pNoMoveTex = GET_SINGLE(CResourcesManager)->LoadTexture(
		"TileNoMove", L"OptionNoMove.bmp");
	...
	m_pStage->CreateTile(100, 100, 52, 52, "BaseTile", L"BaseTile.bmp");

	CTexture* pTex = GET_SINGLE(CResourcesManager)->FindTexture("BaseTile");
	m_vecTileTex.push_back(pTex);

	pTex = GET_SINGLE(CResourcesManager)->LoadTexture(
		"Tile1", L"RockTile.bmp");
	m_vecTileTex.push_back(pTex);
	...
	GET_SINGLE(CInput)->AddKey("Load", '0', VK_CONTROL);
...
void CMapEditScene::Input(float fDeltaTime)
{
	...
	if (KEYPRESS("MouseLButton"))
	{
		...
		{
		case TEM_TEXTURE:
			m_pStage->ChangeTileTexture(tMouseWorldPos,
				m_vecTileTex[m_iEditTileTex]);
			break;
		case TEM_OPTION:
