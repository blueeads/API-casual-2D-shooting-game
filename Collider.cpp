...
bool CCollider::CollisionRectToRect(const RECTANGLE & src,
...
bool CCollider::CollisionRectToSphere(const RECTANGLE & src,
	const SPHERE & dest)
{
	// 원의 중점의 x좌표가 사각형의 가로 영역 안에 있는지,
	// 원의 중점의 y좌표가 사각형의 세로 영역 안에 있는지 체크.
	// 둘 중 하나라도 맞다면 사각형의 위, 아래, 좌, 우 영역 안에
	// 존재한다는 것이다.
...
bool CCollider::CollisionSphereToSphere(const SPHERE & src,
	const SPHERE & dest)
{
	float fDist = CMath::Distance(src.tCenter,
		dest.tCenter);
...
bool CCollider::CollisionSphereToPixel(const SPHERE & src,
...
bool CCollider::CollisionPixelToPoint(const vector<PIXEL>& vecPixel,
